...
(function($){
	$.fn.etalage = function(options){
		...
		var o = $.extend({
			...
			speed: 400,							// All animation speeds are based on this setting (value in ms)
...
//				$thumbs.css({ width:o.thumb_image_width, height:o.thumb_image_height });
				...
				// Add magnifier
				var $magnifier = $('<li class="etalage_magnifier'+(o.show_lens?"":" hidden")+'"><div><img /></div></li>').appendTo($container),
...
// FUNCTIONS
				...
				var start_zoom = function(){
					...
					$magnifier.stop().show().find("img").hide();
...
// EXTERNAL FUNCTIONS
				...
				// Stop Auto Play
				window[instance_id+'_stopAutoplay'] = function(){
					stopAutoplay();
				};
// CALLBACK FUNCTIONS
				...
				var click_callback = function(image_anchor,element){
					// Plugin option callback
					if(!o.click_callback(image_anchor, instance_id,element)){
						return false;
					}
					// Legacy support
					if(typeof etalage_click_callback === 'function'){
						etalage_click_callback(image_anchor, instance_id,element);
						return false;
					}
					return true;
				};
	...
	st_slide_to_startend('end');
				...
				// Large thumbnail click callback
					...
					$magnifier.click(function(){
						var image = $current_thumb;
						var image_anchor = $current_thumb.data('anchor');
						if(image_anchor){
							// Click callback function
							if(click_callback(image_anchor,image)){
								// Regular href action
